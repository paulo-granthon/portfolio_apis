...
export default function ContributionList({ projectId, userId }: ContributionListProps) {
  const [contributions, setContributions] = useState<ContributionSchema[]>([]);

  useEffect(() => {
    getContributionsOfUserProject(userId, projectId).then(contributions => setContributions(contributions));
  }, [projectId, userId]);
